  ...
  const handleOAuthLogin = async (provider: 'google' | 'github') => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider,
        options: {
          redirectTo: `${window.location.origin}/admin` // We will let the router handle role redirection later
        }
      });
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <button 
                ...
                className="flex items-center justify-center gap-3 px-4 py-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors text-white font-medium group"
              >
