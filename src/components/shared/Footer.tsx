...
export const Footer = () => {
  ...
  return (
    <footer className="bg-[#040811] text-zinc-400 py-12 md:py-16 lg:py-20 border-t border-white/5">
              ...
              {footerData.social.map((item) => {
                ...
                return (
                  <Button
                    ...
                    href={item.href}
                    variant="ghost"
                    className="text-zinc-400 hover:text-white hover:-translate-y-1 hover:bg-transparent transition-all duration-300 focus-visible:ring-2 focus-visible:ring-purple-500 rounded-sm p-1 w-auto h-auto min-h-0"
            ...
            <ul className="flex flex-col space-y-3">
              {footerData.services.map((link) => (
                <li key={link.name}>
                  <Link
                    to={link.href}
                    className="text-sm text-zinc-400 hover:text-white transition-colors focus:outline-none focus:ring-1 focus:ring-purple-500 rounded-sm"
                  >
                    {link.name}
                  </Link>
                </li>
            ...
            <ul className="flex flex-col space-y-4">
              <li>
                <a href={`mailto:${footerData.contact.email}`} className="flex items-center space-x-3 group focus:outline-none focus:ring-1 focus:ring-purple-500 rounded-sm">
                  ...
                  <span className="text-sm text-zinc-400 group-hover:text-white transition-colors">{footerData.contact.email}</span>
                </a>
              </li>
              <li>
                <a href={`tel:${footerData.contact.phone.replace(/[^0-9+]/g, '')}`} className="flex items-center space-x-3 group focus:outline-none focus:ring-1 focus:ring-purple-500 rounded-sm">
                  <Phone className="w-4 h-4 text-cyan-400 group-hover:text-white transition-colors" />
                  <span className="text-sm text-zinc-400 group-hover:text-white transition-colors">{footerData.contact.phone}</span>
                </a>
              </li>
              <li className="flex items-start space-x-3">
                <MapPin className="w-4 h-4 text-cyan-400 mt-1 shrink-0" />
                <span className="text-sm text-zinc-400 leading-relaxed">{footerData.contact.address}</span>
              </li>
            </ul>
          </div>
