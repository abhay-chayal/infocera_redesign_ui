...
export const TechnologyCard = ({ title, icon: Icon, techs }: TechnologyCardProps) => {
  return (
    <Link to="/services" className="block h-full group">
      ...
      <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-gradient-to-tr from-indigo-500/10 to-transparent blur-3xl rounded-full opacity-0 group-hover:opacity-100 transition-all duration-1000 pointer-events-none" />
      ...
      <div className="relative z-10 flex flex-wrap gap-2.5 mt-auto">
        {techs.map((tech, idx) => (
          <span 
            key={idx}
            className="px-3.5 py-1.5 text-xs font-semibold text-gray-400 bg-[#0B1120]/50 border border-white/5 rounded-md hover:border-indigo-400/50 hover:bg-indigo-500/10 hover:text-white transition-all duration-300 tracking-wide cursor-default hover:-translate-y-0.5"
          >
            {tech}
          </span>
        ))}
